...
class NghiDinhGraphBuilder:
    def __init__(self, uri, user, password):
        ...
        # ID gốc và các thông tin chung của Nghị định 123/2015/NĐ-CP
        self.base_id = "NghiDinh_123_2015_ND_CP"
        self.common_props = {
            "loai_van_ban": "Nghị Định",
            "ngay_ban_hanh": "15/11/2015",
    ...
    def parse_and_insert(self, file_path):
        ...
        # Chỉ lấy các dòng có chữ
        lines = [p.text.strip() for p in doc.paragraphs if p.text.strip() != ""]

        # Thông tin của node gốc NghiDinh
        nghi_dinh_info = {
            "id": self.base_id,
            "so_hieu": "123/2015/NĐ-CP",
            "ten_van_ban": "Nghị định quy định chi tiết một số điều và biện pháp thi hành Luật Hộ tịch",
            "co_quan_ban_hanh": "Chính phủ",
            "noidung": "Nghị định quy định chi tiết một số điều và biện pháp thi hành Luật Hộ tịch"
        }
        ...
        for line in lines:
            ...
            # Gộp text đa dòng cho entity đang kích hoạt
            if active_entity is not None:
                active_entity["noidung"] += " " + line

        print(f"Đã phân rã: {len(articles)} Điều, {len(clauses)} Khoản, {len(points)} Điểm.")
...
if __name__ == "__main__":
    ...
    # Bạn đổi tên file Word tại đây (Lưu ý: phải chuyển sang .docx để python-docx đọc được)
    base_dir = os.path.dirname(os.path.abspath(__file__))
    FILE_PATH = os.path.join(base_dir, "123_2015_ND-CP_282304.docx")
