...
class NghiDinhGraphBuilder:
    def __init__(self, uri, user, password):
        ...
        # ID gốc và các thông tin chung của Nghị định 07/2025/NĐ-CP
        self.base_id = "NghiDinh_07_2025_ND_CP"
        self.common_props = {
    ...
    def parse_and_insert(self, file_path):
        ...
        nghi_dinh_info = {
            "id": self.base_id,
            "so_hieu": "07/2025/NĐ-CP",
            "ten_van_ban": "Nghị định Sửa đổi, bổ sung một số điều của các Nghị định trong lĩnh vực hộ tịch, quốc tịch, chứng thực",
            "co_quan_ban_hanh": "Chính phủ",
            "noidung": "Nghị định Sửa đổi, bổ sung một số điều của các Nghị định trong lĩnh vực hộ tịch, quốc tịch, chứng thực"
        }
        ...
        is_quoting = False  # Trạng thái đang nằm trong trích đoạn "..."

        # Regex nhận diện các cấp bậc chính của văn bản hiện tại
        ...
        for line in lines:
            # XỬ LÝ TRONG TRÍCH ĐOẠN
            if is_quoting:
                if active_entity:
                    active_entity["noidung"] += "\n" + line
                
                # Kiểm tra kết thúc trích đoạn: dấu .” hoặc ;” ở cuối dòng
                if line.endswith('.”.') or line.endswith(';”.'):
                    is_quoting = False
                continue

            # XỬ LÝ KHI KHÔNG TRONG TRÍCH ĐOẠN
            # 1. Kiểm tra nếu dòng bắt đầu trích đoạn bằng dấu “
            if line.startswith('“'):
                is_quoting = True
                if active_entity:
                    active_entity["noidung"] += "\n" + line
                
                # Trường hợp trích đoạn ngắn kết thúc ngay trong dòng đó
                if line.endswith('.”.') or line.endswith(';”.'):
                    is_quoting = False
                continue

            # 2. Nhận diện các thành phần cấu trúc chính
            # Chương
            ...
            if dieu_match := dieu_pattern.match(line):
                ...
                art_dict = {
                    ...
                    "noidung": line, "nghi_dinh_id": self.base_id
                }
            ...
            # Nội dung bình thường (không phải tiêu đề, không phải mở đầu trích đoạn)
            if active_entity:
        ...
        print(f"Đã hoàn tất bóc tách Nghị định {self.base_id}. Tổng cộng: {len(articles)} Điều.")

    @staticmethod
    def _upsert_nghi_dinh(tx, info, common):
        query = """
        MERGE (nd:NghiDinh {id: $info.id})
        SET nd.so_hieu = $info.so_hieu, nd.ten_van_ban = $info.ten_van_ban, nd.co_quan_ban_hanh = $info.co_quan_ban_hanh,
            nd.noidung = $info.noidung, nd.ngay_ban_hanh = $common.ngay_ban_hanh, nd.loai_van_ban = $common.loai_van_ban,
            nd.ngay_co_hieu_luc = $common.ngay_co_hieu_luc, nd.ngay_het_hieu_luc = $common.ngay_het_hieu_luc,
            nd.tinh_trang_hieu_luc = $common.tinh_trang_hieu_luc, nd.cap_bac_phap_ly = $common.cap_bac_phap_ly
        """
...
if __name__ == "__main__":
    import os
    import sys
    # Thiết lập đường dẫn để import config
    ...
    FILE_PATH = os.path.join(base_dir, "Nghị-định-07-2025-NĐ-CP.docx")
