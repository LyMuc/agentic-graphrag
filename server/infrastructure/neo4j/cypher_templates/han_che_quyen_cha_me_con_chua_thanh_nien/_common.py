"""Common Cypher snippets + helpers cho topic hạn chế quyền cha mẹ (han_che_quyen)."""
from __future__ import annotations

from server.infrastructure.neo4j.cypher_templates._context_tho_common import (
...
GENERIC_ID_SEED_BODY = f"""
WITH $leaf_ids AS leaf_ids, $extra_seed_ids AS extra_ids, $whitelist_dieu_ids AS wl

OPTIONAL MATCH (leaf:{TOPIC_LABEL})
WHERE leaf.id IN leaf_ids AND leaf.topic = '{TOPIC}'

OPTIONAL MATCH (extra:{TOPIC_LABEL})
WHERE extra.id IN extra_ids AND extra.topic = '{TOPIC}'

WITH wl,
  [x IN collect(DISTINCT leaf) + collect(DISTINCT extra) WHERE x IS NOT NULL] AS seed_nodes,
