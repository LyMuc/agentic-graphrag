"""Chuẩn hoá ``Context_Tho`` thành văn bản căn cứ pháp lý cho LLM.

Tách từ ``utils/utils.py`` trong Phase 1 refactor (domain layer). Hai hàm I/O
``_fetch_tvpl_links`` / ``_fetch_provision_effective_dates`` vẫn nạp lazy
``adapter.config.driver``; Phase 2 sẽ trỏ sang
``server.infrastructure.neo4j.client``.
"""
