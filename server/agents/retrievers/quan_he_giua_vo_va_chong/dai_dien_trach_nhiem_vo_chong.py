...
from server.infrastructure.neo4j.cypher_templates.dai_dien_trach_nhiem_vo_chong.term_mapping import (
    resolve_term,
)
...
class DaiDienTrachNhiemVoChongRetriever(RetrieverAgent):
    """Subagent ``dai_dien_trach_nhiem_vo_chong`` — thin wrapper ủy quyền pipeline classify→extract→execute→run."""
    ...
    async def run(self, query: str):
        return await dai_dien_trach_nhiem_vo_chong(query)
