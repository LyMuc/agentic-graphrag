...
from server.infrastructure.neo4j.cypher_templates.tai_san.term_mapping import (
...
class CheDoTaiSanCuaVoChongRetriever(RetrieverAgent):
    """Subagent ``che_do_tai_san_cua_vo_chong`` — thin wrapper ủy quyền pipeline classify→extract→execute→run."""
    ...
    async def run(self, query: str):
        return await che_do_tai_san_cua_vo_chong(query)
