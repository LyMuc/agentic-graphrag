...
from server.infrastructure.neo4j.cypher_templates.hon_nhan_cham_dut_do_vo_chong_chet.term_mapping import (
    resolve_term,
)
...
class HonNhanChamDutDoVoChongChetRetriever(RetrieverAgent):
    """Subagent ``hon_nhan_cham_dut_do_vo_chong_chet`` — thin wrapper ủy quyền pipeline classify→extract→execute→run."""
    ...
    async def run(self, query: str):
        return await hon_nhan_cham_dut_do_vo_chong_chet(query)
