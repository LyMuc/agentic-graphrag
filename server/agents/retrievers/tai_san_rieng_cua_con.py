...
from server.infrastructure.neo4j.cypher_templates.tai_san_rieng_cua_con import TAI_SAN_RIENG_CUA_CON_REGISTRY
from server.infrastructure.neo4j.cypher_templates.tai_san_rieng_cua_con.term_mapping import resolve_term
...
class TaiSanRiengCuaConRetriever(RetrieverAgent):
    """Subagent ``tai_san_rieng_cua_con`` — thin wrapper ủy quyền pipeline classify→extract→execute→run."""
    ...
    async def run(self, query: str):
        return await tai_san_rieng_cua_con(query)
