...
from server.infrastructure.neo4j.cypher_templates.chia_tai_san_sau_ly_hon import (
    CHIA_TAI_SAN_SAU_LY_HON_REGISTRY,
)
from server.infrastructure.neo4j.cypher_templates.chia_tai_san_sau_ly_hon.term_mapping import resolve_term
...
async def chia_tai_san_sau_ly_hon(query: str) -> dict[str, Any]:
    """Truy xuất căn cứ giải quyết tài sản khi hoặc sau ly hôn.

    Args:
        query: Câu hỏi pháp lý đã được Router giải nghĩa đầy đủ ngữ cảnh.

    Returns:
        Dictionary gồm ``contexts`` là LegalContextBundle đã encode, ``debug``
        của các template và ``graph_viz_id`` tùy chọn. Lỗi từng template được
        giữ dưới dạng context text thay vì làm thất bại toàn bộ retriever.
    """
        ...
        try:
            contexts.append(
                encode_context_record(
                    ...
                    retriever_name="chia_tai_san_sau_ly_hon",
                    template_name=template.name,
                )
            )
        except Exception as exc:
...
class ChiaTaiSanSauLyHonRetriever(RetrieverAgent):
    """Subagent ``chia_tai_san_sau_ly_hon`` — thin wrapper ủy quyền pipeline classify→extract→execute→run."""
    ...
    async def run(self, query: str):
        return await chia_tai_san_sau_ly_hon(query)
