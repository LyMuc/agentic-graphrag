...
from server.infrastructure.neo4j.cypher_templates.xac_dinh_cha_me_con import XAC_DINH_CHA_ME_CON_REGISTRY
from server.infrastructure.neo4j.cypher_templates.xac_dinh_cha_me_con.term_mapping import resolve_term
        ...
        try:
            contexts.append(
                encode_context_record(
                    ...
                    retriever_name="xac_dinh_cha_me_con",
                    template_name=template.name,
                )
            )
        except Exception as exc:
...
class XacDinhChaMeConRetriever(RetrieverAgent):
    """Subagent ``xac_dinh_cha_me_con`` — thin wrapper ủy quyền pipeline classify→extract→execute→run."""
    ...
    async def run(self, query: str):
        return await xac_dinh_cha_me_con(query)
