...
from server.infrastructure.neo4j.cypher_templates.quan_he_giua_cac_thanh_vien_khac_trong_gia_dinh.term_mapping import (
    resolve_term,
)
...
class QuanHeGiuaCacThanhVienKhacTrongGiaDinhRetriever(RetrieverAgent):
    """Subagent ``quan_he_giua_cac_thanh_vien_khac_trong_gia_dinh`` — thin wrapper ủy quyền pipeline classify→extract→execute→run."""
    ...
    async def run(self, query: str):
        return await quan_he_giua_cac_thanh_vien_khac_trong_gia_dinh(query)
