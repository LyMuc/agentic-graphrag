...
from server.infrastructure.neo4j.cypher_templates.quyen_nghia_vu_cha_me_con.term_mapping import resolve_term
        ...
        try:
            contexts.append(
                encode_context_record(
                    ...
                    retriever_name="quyen_nghia_vu_cha_me_con",
                    template_name=template.name,
                )
            )
        except Exception as exc:
...
class QuyenNghiaVuChaMeConRetriever(RetrieverAgent):
    """Subagent ``quyen_nghia_vu_cha_me_con`` — thin wrapper ủy quyền pipeline classify→extract→execute→run."""
    ...
    async def run(self, query: str):
        return await quyen_nghia_vu_cha_me_con(query)
