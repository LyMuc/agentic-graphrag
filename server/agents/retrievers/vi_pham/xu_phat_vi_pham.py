...
from server.infrastructure.neo4j.cypher_templates.xu_phat_vi_pham.term_mapping import resolve_term
...
class XuPhatViPhamRetriever(RetrieverAgent):
    """Subagent ``xu_phat_vi_pham`` — thin wrapper ủy quyền pipeline classify→extract→execute→run."""
    ...
    async def run(self, query: str):
        return await xu_phat_vi_pham(query)
