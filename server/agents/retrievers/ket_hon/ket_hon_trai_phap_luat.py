...
from server.infrastructure.neo4j.cypher_templates.ket_hon_trai_phap_luat.term_mapping import resolve_term
        ...
        try:
            contexts.append(
                encode_context_record(
                    ...
                    retriever_name="ket_hon_trai_phap_luat",
                    template_name=template.name,
                )
            )
        except Exception as exc:
...
class KetHonTraiPhapLuatRetriever(RetrieverAgent):
    """Subagent ``ket_hon_trai_phap_luat`` — thin wrapper ủy quyền pipeline classify→extract→execute→run."""
    ...
    async def run(self, query: str):
        return await ket_hon_trai_phap_luat(query)
