...
from server.infrastructure.neo4j.cypher_templates.dieu_kien_ket_hon.term_mapping import resolve_term
...
class DieuKienKetHonRetriever(RetrieverAgent):
    """Subagent ``dieu_kien_ket_hon`` — thin wrapper ủy quyền pipeline classify→extract→execute→run."""
    ...
    async def run(self, query: str):
        return await dieu_kien_ket_hon(query)
