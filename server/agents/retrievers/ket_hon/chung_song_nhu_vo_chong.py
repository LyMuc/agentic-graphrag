...
from server.infrastructure.neo4j.cypher_templates.chung_song_nhu_vo_chong import (
    CHUNG_SONG_NHU_VO_CHONG_REGISTRY,
)
from server.infrastructure.neo4j.cypher_templates.chung_song_nhu_vo_chong.term_mapping import resolve_term
...
class ChungSongNhuVoChongRetriever(RetrieverAgent):
    """Subagent ``chung_song_nhu_vo_chong`` — thin wrapper ủy quyền pipeline classify→extract→execute→run."""
    ...
    async def run(self, query: str):
        return await chung_song_nhu_vo_chong(query)
