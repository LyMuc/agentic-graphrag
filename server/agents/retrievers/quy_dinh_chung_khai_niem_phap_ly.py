...
from server.infrastructure.neo4j.cypher_templates.quy_dinh_chung_khai_niem_phap_ly.term_mapping import (
    resolve_term,
)
        ...
        try:
            contexts.append(
                encode_context_record(
                    ...
                    retriever_name="quy_dinh_chung_khai_niem_phap_ly",
                    template_name=template.name,
                )
            )
        except Exception as exc:
...
class QuyDinhChungKhaiNiemPhapLyRetriever(RetrieverAgent):
    """Subagent ``quy_dinh_chung_khai_niem_phap_ly`` — thin wrapper ủy quyền pipeline classify→extract→execute→run."""
    ...
    async def run(self, query: str):
        return await quy_dinh_chung_khai_niem_phap_ly(query)
